...
export class PgStatement {
  ...
  async run(...params: unknown[]): Promise<RunResult> {
    ...
    const skipReturningId =
      isInsert &&
      !hasReturning &&
      /INSERT\s+INTO\s+("?ROLE_ESCRITURA"?|"?ROLE_PERMISOS"?|"?RUBRO_SUB_RUBROS"?|"?USER_SESSIONS"?|"?STOCK_GANADERO_DISPOSITIVO"?|"?STOCK_GANADERO_RAZA"?|"?STOCK_GANADERO_POTRERO"?|"?STOCK_GANADERO_GRUPO"?|"?STOCK_EQUINO_DISPOSITIVO"?|"?CHAT_CHANNEL_MEMBERS"?|"?CHAT_READ_STATE"?|"?CHAT_WALLPAPER"?|"?SIMULADOR_VENTA_GANADO_OP_SEQ"?|"?PRESUPUESTO_DOCUMENTOS"?|"?USER_VENCIMIENTOS_PREFS"?)\b/i.test(
        sql
      );
