...
import {
  ...
  OVINO_FRONTERA_ADULTO,
  OVINO_ID_SEED_ULTIMO,
  labelCategoriaSalidaDispositivo,
  splitOvinoIdInterno,
  type CategoriaOvino,
} from "./stock-ovina-categoria.js";
...
export type { CategoriaOvino } from "./stock-ovina-categoria.js";

export type StockOvinoRowInput = StockGanaderoRowInput;

export interface StockOvinoLote {
  id: number;
...
export interface StockOvinoRegistro {
  id: number;
  lote_id: number;
  eid: string;
  vid: string;
...
export interface StockOvinoFilters {
  lote_id?: number;
  busqueda?: string;
  fecha_desde?: string;
  fecha_hasta?: string;
...
export interface StockOvinoEidRepetido {
  eid: string;
  vid: string;
  clave: string;
  cantidad: number;
...
export interface StockOvinoEstadisticas {
  total_lecturas: number;
  eids_activos: number;
  eids_repetidos: number;
  lecturas_en_repetidos: number;
  detalle_repetidos: StockOvinoEidRepetido[];
}
...
function appendRegistroFilters(
  ...
  if (filters?.busqueda?.trim()) {
    sql += ` AND (
      ...
      EXISTS (
        SELECT 1 FROM STOCK_OVINO_DISPOSITIVO d
        WHERE d.clave = regexp_replace(COALESCE(${p}eid, '') || COALESCE(${p}vid, ''), '[^0-9]', '', 'g')
          AND (
            ...
            d.raza LIKE @busqueda OR
            d.pelaje LIKE @busqueda OR
  ...
  if (filters?.cuenta_id != null) {
    sql += ` AND ${p}lote_id IN (SELECT id FROM STOCK_OVINO_LOTE WHERE cuenta_id = @cuenta_id)`;
...
async function clavesEidRepetidas(
  ...
  let sql = `SELECT eid, vid FROM STOCK_OVINO_REGISTRO WHERE 1=1`;
...
export async function initStockOvinoTables(db: Db): Promise<void> {
  await ensureStockOvinoBaseTables(db);
  await migrateStockOvinoLoteCuenta(db);
  await migrateGrupoLibreColumn(db);
  await migrateBajaMetaColumns(db);
  await stockFoto.migrateStockDispositivoFotoColumns(db, "ovino");
  await stockFoto.migrateStockDispositivoFotosGallery(db, "ovino");
  await stockFoto.migrateStockDispositivoFotosBlobColumns(db, "ovino");
  await stockControlSanitario.migrateStockControlSanitarioTable(db, "ovino");
  ...
  await migrateStockOvinoDispositivoHistorial(db);
  await migrateHistorialAutorColumns(db);
  await potreroDb.migrateOvinoPotreroColumn(db);
  await migrateOvinoAltaGenericaColumns(db);
  await migrateOvinoCabanaColumns(db);
  await migrateOvinoRazaColumn(db);
  await migrateOvinoRazaCatalog(db);
  await migrateOvinoPelajeColumn(db);
  await migrateOvinoPelajeCatalog(db);
  await ensureStockOvinoIdSeq(db);
}

async function migrateOvinoAltaGenericaColumns(db: Db): Promise<void> {
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
  ...
  for (const sql of [
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN categoria TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN castrado INTEGER`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN origen_alta TEXT NOT NULL DEFAULT ''`,
  ]) {
    try {
  ...
  await db
    .prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO_HISTORIAL
...
async function migrateOvinoCabanaColumns(db: Db): Promise<void> {
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
  ...
  for (const sql of [
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN rp TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN nombre_animal TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN registro TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN premios TEXT NOT NULL DEFAULT ''`,
  ]) {
    try {
      await db.prepare(sql).run();
    } catch {
...
async function migrateOvinoRazaColumn(db: Db): Promise<void> {
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
       WHERE clave = '__meta__' AND campo = 'raza_col_v1' LIMIT 1`
    )
    .get()) as { ok: number } | undefined;
  if (done) return;
  ...
  try {
    await db
      .prepare(
        `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN raza TEXT NOT NULL DEFAULT ''`
      )
      .run();
  } catch {
    /* columna ya existe */
  ...
  await db
    .prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO_HISTORIAL
         (clave, campo, etiqueta, valor_anterior, valor_nuevo)
       VALUES ('__meta__', 'raza_col_v1', '', '', '')`
    )
    .run();
}

function normalizarRazaOvino(val: string | null | undefined): string {
  return String(val ?? "")
    .trim()
...
const RAZAS_OVINO_CATALOGO_INICIALES = ["CRIOLLA"] as const;

async function migrateOvinoRazaCatalog(db: Db): Promise<void> {
  await db
    .prepare(
      `CREATE TABLE IF NOT EXISTS STOCK_OVINO_RAZA (
  ...
  for (const nombre of RAZAS_OVINO_CATALOGO_INICIALES) {
    await db
      .prepare(
        `INSERT INTO STOCK_OVINO_RAZA (nombre) VALUES (?) ON CONFLICT(nombre) DO NOTHING`
      )
      .run(nombre);
  }
  ...
  try {
    const usadas = (await db
      .prepare(
        `SELECT DISTINCT raza FROM STOCK_OVINO_DISPOSITIVO
         WHERE raza IS NOT NULL AND TRIM(raza) <> ''`
      )
      .all()) as { raza: string }[];
    for (const row of usadas) {
      const nombre = normalizarRazaOvino(row.raza);
      if (!nombre) continue;
      await db
        .prepare(
          `INSERT INTO STOCK_OVINO_RAZA (nombre) VALUES (?) ON CONFLICT(nombre) DO NOTHING`
    ...
    /* columna raza puede no existir aún en entornos raros */
  }
...
export async function listStockOvinoRazas(db: Db): Promise<string[]> {
  const rows = (await db
    .prepare(`SELECT nombre FROM STOCK_OVINO_RAZA ORDER BY nombre ASC`)
    .all()) as { nombre: string }[];
  return rows.map((r) => normalizarRazaOvino(r.nombre)).filter(Boolean);
}

export async function createStockOvinoRaza(db: Db, raw: string): Promise<string> {
  const nombre = normalizarRazaOvino(raw);
  if (!nombre) {
    throw new Error("Ingresá un nombre de raza válido (letras y números).");
  }
  const existe = (await db
    .prepare(`SELECT 1 AS ok FROM STOCK_OVINO_RAZA WHERE nombre = ? LIMIT 1`)
    .get(nombre)) as { ok: number } | undefined;
  if (existe) return nombre;
  await db.prepare(`INSERT INTO STOCK_OVINO_RAZA (nombre) VALUES (?)`).run(nombre);
  return nombre;
}

export async function ensureStockOvinoRazaEnCatalogo(
  db: Db,
  raw: string | null | undefined
): Promise<string> {
  const nombre = normalizarRazaOvino(raw);
  if (!nombre) return "";
  return createStockOvinoRaza(db, nombre);
}

export async function deleteStockOvinoRaza(db: Db, raw: string): Promise<string> {
  const nombre = normalizarRazaOvino(raw);
  if (!nombre) throw new Error("Raza inválida");
  const usos = (await db
    .prepare(`SELECT COUNT(*) AS n FROM STOCK_OVINO_DISPOSITIVO WHERE raza = ?`)
    .get(nombre)) as { n: number | string };
  const nUsos = Number(usos?.n ?? 0);
  if (nUsos > 0) {
    throw new Error(
      `No se puede eliminar «${nombre}»: está asignada a ${nUsos} ovino(s).`
    );
  }
  const result = await db
    .prepare(`DELETE FROM STOCK_OVINO_RAZA WHERE nombre = ?`)
    .run(nombre);
  if (!result.changes) throw new Error(`No se encontró la raza «${nombre}».`);
  return nombre;
}

function normalizarPelajeOvino(val: string | null | undefined): string {
  return normalizarRazaOvino(val);
}

const PELAJES_EQUINO_CATALOGO_INICIALES = [
  "ALAZÁN",
  "BAYO",
  "ZAINO",
  "NEGRO",
...
async function migrateOvinoPelajeColumn(db: Db): Promise<void> {
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
       WHERE clave = '__meta__' AND campo = 'pelaje_col_v1' LIMIT 1`
    )
    .get()) as { ok: number } | undefined;
  if (done) return;
  ...
  try {
    await db
      .prepare(
        `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN pelaje TEXT NOT NULL DEFAULT ''`
      )
      .run();
  } catch {
    /* columna ya existe */
  ...
  await db
    .prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO_HISTORIAL
         (clave, campo, etiqueta, valor_anterior, valor_nuevo)
       VALUES ('__meta__', 'pelaje_col_v1', '', '', '')`
    )
    .run();
}

async function migrateOvinoPelajeCatalog(db: Db): Promise<void> {
  await db
    .prepare(
      `CREATE TABLE IF NOT EXISTS STOCK_OVINO_PELAJE (
         nombre TEXT PRIMARY KEY,
         creado_en TIMESTAMPTZ DEFAULT NOW()
       )`
    )
  ...
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
       WHERE clave = '__meta__' AND campo = 'pelaje_catalog_v1' LIMIT 1`
    )
    .get()) as { ok: number } | undefined;

  for (const nombre of PELAJES_EQUINO_CATALOGO_INICIALES) {
    await db
      .prepare(
        `INSERT INTO STOCK_OVINO_PELAJE (nombre) VALUES (?) ON CONFLICT(nombre) DO NOTHING`
      )
      .run(nombre);
  }
  ...
  try {
    const usadas = (await db
      .prepare(
        `SELECT DISTINCT pelaje FROM STOCK_OVINO_DISPOSITIVO
         WHERE pelaje IS NOT NULL AND TRIM(pelaje) <> ''`
      )
      .all()) as { pelaje: string }[];
    for (const row of usadas) {
      const nombre = normalizarPelajeOvino(row.pelaje);
      if (!nombre) continue;
      await db
        .prepare(
          `INSERT INTO STOCK_OVINO_PELAJE (nombre) VALUES (?) ON CONFLICT(nombre) DO NOTHING`
    ...
    /* columna pelaje puede no existir aún */
  }
  ...
  await db
    .prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO_HISTORIAL
         (clave, campo, etiqueta, valor_anterior, valor_nuevo)
       VALUES ('__meta__', 'pelaje_catalog_v1', '', '', '')`
    )
    .run();
}

export async function listStockOvinoPelajes(db: Db): Promise<string[]> {
  const rows = (await db
    .prepare(`SELECT nombre FROM STOCK_OVINO_PELAJE ORDER BY nombre ASC`)
    .all()) as { nombre: string }[];
  return rows.map((r) => normalizarPelajeOvino(r.nombre)).filter(Boolean);
}

export async function createStockOvinoPelaje(db: Db, raw: string): Promise<string> {
  const nombre = normalizarPelajeOvino(raw);
  if (!nombre) {
    throw new Error("Ingresá un nombre de pelaje válido (letras y números).");
  }
  const existe = (await db
    .prepare(`SELECT 1 AS ok FROM STOCK_OVINO_PELAJE WHERE nombre = ? LIMIT 1`)
    .get(nombre)) as { ok: number } | undefined;
  if (existe) return nombre;
  await db.prepare(`INSERT INTO STOCK_OVINO_PELAJE (nombre) VALUES (?)`).run(nombre);
  return nombre;
}

export async function ensureStockOvinoPelajeEnCatalogo(
  db: Db,
  raw: string | null | undefined
): Promise<string> {
  const nombre = normalizarPelajeOvino(raw);
  if (!nombre) return "";
  return createStockOvinoPelaje(db, nombre);
}

export async function deleteStockOvinoPelaje(db: Db, raw: string): Promise<string> {
  const nombre = normalizarPelajeOvino(raw);
  if (!nombre) throw new Error("Pelaje inválido");
  const usos = (await db
    .prepare(`SELECT COUNT(*) AS n FROM STOCK_OVINO_DISPOSITIVO WHERE pelaje = ?`)
    .get(nombre)) as { n: number | string };
  const nUsos = Number(usos?.n ?? 0);
  if (nUsos > 0) {
    throw new Error(
      `No se puede eliminar «${nombre}»: está asignado a ${nUsos} ovino(s).`
    );
  }
  const result = await db
    .prepare(`DELETE FROM STOCK_OVINO_PELAJE WHERE nombre = ?`)
    .run(nombre);
  if (!result.changes) throw new Error(`No se encontró el pelaje «${nombre}».`);
  return nombre;
}

async function ensureStockOvinoIdSeq(db: Db): Promise<void> {
  await db
    .prepare(
      `CREATE TABLE IF NOT EXISTS STOCK_OVINO_ID_SEQ (
  ...
  const row = (await db
    .prepare(`SELECT ultimo FROM STOCK_OVINO_ID_SEQ WHERE id = 1`)
    .get()) as { ultimo: string } | undefined;
  if (!row) {
    await db
      .prepare(`INSERT INTO STOCK_OVINO_ID_SEQ (id, ultimo) VALUES (1, ?)`)
      .run(OVINO_ID_SEED_ULTIMO);
  }
}

/** Reserva IDs correlativos globales (prefijo 199). Misma secuencia para genéricos y cabaña. */
async function reservarIdsOvinos(tx: Db, cantidad: number): Promise<string[]> {
  await ensureStockOvinoIdSeq(tx);
  const seqRow = (await tx
    .prepare(`SELECT ultimo FROM STOCK_OVINO_ID_SEQ WHERE id = 1 FOR UPDATE`)
    .get()) as { ultimo: string } | undefined;
  if (!seqRow?.ultimo) {
    throw new Error("No se pudo reservar la secuencia de IDs ovinos.");
  }
  const claves: string[] = [];
  ...
  await tx
    .prepare(`UPDATE STOCK_OVINO_ID_SEQ SET ultimo = ? WHERE id = 1`)
    .run(cursor);
  return claves;
}

async function ensureStockOvinoBaseTables(db: Db): Promise<void> {
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS STOCK_OVINO_LOTE (
  ...
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS STOCK_OVINO_REGISTRO (
      id SERIAL PRIMARY KEY,
      lote_id INTEGER NOT NULL REFERENCES STOCK_OVINO_LOTE(id) ON DELETE CASCADE,
      eid TEXT NOT NULL,
  ...
  await db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_stock_ovino_reg_lote ON STOCK_OVINO_REGISTRO(lote_id)
  `).run();
  await db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_stock_ovino_reg_eid ON STOCK_OVINO_REGISTRO(eid)
  `).run();
  await db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_stock_ovino_reg_fecha ON STOCK_OVINO_REGISTRO(fecha)
  `).run();

  await db.prepare(`
    CREATE TABLE IF NOT EXISTS STOCK_OVINO_DISPOSITIVO (
  ...
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS STOCK_OVINO_DISPOSITIVO_HISTORIAL (
      id SERIAL PRIMARY KEY,
      clave TEXT NOT NULL,
      campo TEXT NOT NULL,
  ...
  await db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_stock_ovino_disp_hist_clave
      ON STOCK_OVINO_DISPOSITIVO_HISTORIAL(clave)
  `).run();
  await db.prepare(`
    CREATE INDEX IF NOT EXISTS idx_stock_ovino_disp_hist_fecha
      ON STOCK_OVINO_DISPOSITIVO_HISTORIAL(creado_en)
  `).run();
}

async function migrateStockOvinoLoteCuenta(db: Db): Promise<void> {
  await migrateAddCuentaIdColumn(db, "STOCK_OVINO_LOTE");
}
...
async function migrateHistorialAutorColumns(db: Db): Promise<void> {
  ...
  for (const col of [
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO_HISTORIAL ADD COLUMN user_id INTEGER`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO_HISTORIAL ADD COLUMN user_email TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO_HISTORIAL ADD COLUMN user_nombre TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO_HISTORIAL ADD COLUMN origen TEXT NOT NULL DEFAULT ''`,
  ]) {
    try {
...
async function migrateBajaMetaColumns(db: Db): Promise<void> {
  ...
  for (const col of [
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN tipo_baja TEXT NOT NULL DEFAULT ''`,
    `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN numero_guia TEXT NOT NULL DEFAULT ''`,
  ]) {
    try {
      await db.prepare(col).run();
    } catch {
...
async function migrateGrupoLibreColumn(db: Db): Promise<void> {
  ...
  try {
    await db
      .prepare(
        `ALTER TABLE STOCK_OVINO_DISPOSITIVO ADD COLUMN grupo_libre TEXT NOT NULL DEFAULT ''`
      )
      .run();
  } catch {
    /* columna ya existe */
...
async function migrateFechasSlashLatam(db: Db): Promise<void> {
  ...
  await db
    .prepare(
      `DELETE FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL WHERE campo = 'condicion'`
    )
    .run();
  await db
    .prepare(
      `DELETE FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
...
async function corregirFechasSlashInvertidas(db: Db): Promise<void> {
  const rows = (await db
    .prepare(
      `SELECT id, eid, vid, fecha, hora FROM STOCK_OVINO_REGISTRO ORDER BY id`
    )
    .all()) as Pick<StockOvinoRegistro, "id" | "eid" | "vid" | "fecha" | "hora">[];

  const porHora = new Map<string, Pick<StockOvinoRegistro, "id" | "eid" | "vid" | "fecha" | "hora">[]>();
  for (const r of rows) {
  ...
  const upd = await db.prepare(
    `UPDATE STOCK_OVINO_REGISTRO SET fecha = @fecha WHERE id = @id`
  );
...
async function migrateStockOvinoDispositivoHistorial(db: Db): Promise<void> {
  const done = (await db
    .prepare(
      `SELECT 1 AS ok FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL
...
async function migrateStockOvinoDispositivoMeta(_db: Db): Promise<void> {}

const NACIMIENTO_ANIO_MIN = 1900;
const GRUPO_ANIO_MIN = NACIMIENTO_ANIO_MIN;
...
async function mapMetaDispositivos(
  ...
  for (const row of rows) {
    ...
    map.set(row.clave, {
      ...
      raza: normalizarRazaOvino(row.raza),
      pelaje: normalizarPelajeOvino(row.pelaje),
    });
  }
  return map;
}
...
async function enrichDispositivosWithMeta(
  ...
  const fotos = await stockFoto.mapStockDispositivoFotos(db, "ovino");
...
async function assertDispositivoExiste(db: Db, claveNorm: string): Promise<void> {
  const eids = (await db
    .prepare(`SELECT eid, vid FROM STOCK_OVINO_REGISTRO`)
...
export async function saveStockOvinaDispositivo(
  ...
  const raza = await ensureStockOvinoRazaEnCatalogo(db, input.raza);
  const pelaje = await ensureStockOvinoPelajeEnCatalogo(db, input.pelaje);

  const anteriorRow = (await db
    .prepare(
      `SELECT sexo, empresa, grupo, grupo_libre, potrero, rp, nombre_animal, registro, premios, raza, pelaje,
              nacimiento_mes, nacimiento_anio, observaciones, estado, tipo_baja, numero_guia, baja_mes, baja_anio
       FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`
    )
  ...
  const anterior: DispositivoMetaGuardada | null = anteriorBase
    ? {
        ...
        raza: normalizarRazaOvino(anteriorRow?.raza),
        pelaje: normalizarPelajeOvino(anteriorRow?.pelaje),
      }
    : null;
  ...
  await db.prepare(
    `INSERT INTO STOCK_OVINO_DISPOSITIVO (
       clave, eid, sexo, empresa, grupo, grupo_libre, potrero, raza, pelaje, rp, nombre_animal, registro, premios,
     ...
     ON CONFLICT(clave) DO UPDATE SET
       ...
       eid = CASE WHEN excluded.eid != '' THEN excluded.eid ELSE STOCK_OVINO_DISPOSITIVO.eid END,
...
function mergeMetaConPatch(
  ...
  if (patch.pelaje !== undefined) merged.pelaje = patch.pelaje;
  return merged;
}
  ...
  await db.transaction(async (tx) => {
    ...
    for (const clave of keys) {
      try {
        ...
        await saveStockOvinaDispositivo(tx, claveNorm, input, eid, autor);
        actualizados += 1;
      } catch (e) {
        errores.push({
          clave,
...
export async function deleteStockOvinaDispositivos(
  db: Db,
  claves: string[]
  ...
  await db.transaction(async (tx) => {
    const regRows = (await tx
      .prepare(`SELECT id, eid, vid FROM STOCK_OVINO_REGISTRO`)
      .all()) as { id: number; eid: string; vid: string }[];

    const delRegistro = tx.prepare(`DELETE FROM STOCK_OVINO_REGISTRO WHERE id = ?`);
    const delHistorial = tx.prepare(
      `DELETE FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL WHERE clave = ?`
    );
    const delDispositivo = tx.prepare(`DELETE FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`);
...
export interface VaciarStockOvinaResult {
  dispositivos_eliminados: number;
  lecturas_eliminadas: number;
  vinculos_sim_venta: number;
}

export interface StockOvinaBackupInfo {
  disponible: boolean;
...
export interface RestaurarStockOvinaResult {
...
interface StockOvinaBackupPayload {
  v: 2;
  cuenta_id: number;
  registros: {
    lote_id: number;
...
async function migrateStockOvinaBackupTableSchema(db: Db): Promise<void> {
  const legacy = (await db
    .prepare(
      `SELECT 1 AS ok FROM information_schema.columns
       WHERE table_schema = 'public'
         AND table_name = 'stock_ovino_backup'
         AND column_name = 'id'
       LIMIT 1`
    )
    .get()) as { ok: number } | undefined;
  if (legacy) {
    await db.prepare(`DROP TABLE IF EXISTS STOCK_OVINO_BACKUP`).run();
  }
}

async function ensureStockOvinaBackupTable(db: Db): Promise<void> {
  await migrateStockOvinaBackupTableSchema(db);
  await db
    .prepare(
      `CREATE TABLE IF NOT EXISTS STOCK_OVINO_BACKUP (
...
async function loteIdsPorCuenta(db: Db, cuentaId: number): Promise<Set<number>> {
  const rows = (await db
    .prepare(`SELECT id FROM STOCK_OVINO_LOTE WHERE cuenta_id = ?`)
...
export interface StockOvinoCuentaScope {
  empresaSet: Set<string>;
  clavesLecturas: Set<string>;
}

async function clavesLecturasOvinoPorCuenta(db: Db, cuentaId: number): Promise<Set<string>> {
  const rows = (await db
    .prepare(
      `SELECT r.eid, r.vid
       FROM STOCK_OVINO_REGISTRO r
       INNER JOIN STOCK_OVINO_LOTE l ON l.id = r.lote_id
       WHERE l.cuenta_id = ?`,
    )
...
export async function getStockOvinoCuentaScope(
  db: Db,
  cuentaId: number,
): Promise<StockOvinoCuentaScope> {
  return {
    empresaSet: await empresaCodigosSetPorCuenta(db, cuentaId),
    clavesLecturas: await clavesLecturasOvinoPorCuenta(db, cuentaId),
  };
}

export function stockOvinoDispositivoEnCuenta(
  d: { clave: string; empresa: string },
  scope: StockOvinoCuentaScope,
): boolean {
  return dispositivoPerteneceCuenta(d, scope.empresaSet, scope.clavesLecturas);
}

/** Dispositivos del stock ovino que pertenecen a una cuenta (lotes + empresas operativas). */
export async function listStockOvinaDispositivosPorCuenta(
  db: Db,
  cuentaId: number,
): Promise<StockOvinaDispositivo[]> {
  const scope = await getStockOvinoCuentaScope(db, cuentaId);
  if (scope.empresaSet.size === 0 && scope.clavesLecturas.size === 0) return [];
  const dispositivos = await listStockOvinaDispositivos(db);
  return dispositivos.filter((d) => stockOvinoDispositivoEnCuenta(d, scope));
}

async function gatherBackupPayloadForCuenta(
  db: Db,
  cuentaId: number
): Promise<StockOvinaBackupPayload | null> {
  ...
  const allRegistros = (await db
    .prepare(`SELECT lote_id, eid, vid, fecha, hora, condicion FROM STOCK_OVINO_REGISTRO`)
    .all()) as StockOvinaBackupPayload["registros"];
  ...
  const allDispositivos = (await db
    ...
    .all()) as StockOvinaBackupPayload["dispositivos"];
  ...
  const allHistorial = (await db
    .prepare(
      `SELECT clave, campo, etiqueta, valor_anterior, valor_nuevo, creado_en,
       ...
       FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL`
    )
    .all()) as StockOvinaBackupPayload["historial"];
  ...
  const allSim = (await db
    ...
    .all()) as StockOvinaBackupPayload["sim_dispositivos"];
...
async function crearBackupStockOvina(tx: Db, cuentaId: number): Promise<void> {
  await ensureStockOvinaBackupTable(tx);
  const payload = await gatherBackupPayloadForCuenta(tx, cuentaId);
  if (!payload) return;

  await tx
    .prepare(
      `INSERT INTO STOCK_OVINO_BACKUP (cuenta_id, payload, creado_en)
       VALUES (?, ?, NOW())
...
async function countRegistrosEnCuenta(db: Db, cuentaId: number): Promise<number> {
  ...
  const rows = (await db
    .prepare(`SELECT lote_id FROM STOCK_OVINO_REGISTRO`)
...
async function countDispositivosEnCuenta(db: Db, cuentaId: number): Promise<number> {
  ...
  const filters: StockOvinoFilters = {
    empresas: empresas.length > 0 ? empresas : [SIN_EMPRESAS_SCOPE],
  };
  return countStockOvinaDispositivos(db, filters);
}

async function vaciarStockOvinaDeCuenta(
  tx: Db,
  cuentaId: number
): Promise<VaciarStockOvinaResult> {
  const payload = await gatherBackupPayloadForCuenta(tx, cuentaId);
  if (!payload) {
    return {
  ...
  let lecturas_eliminadas = 0;
  const regRows = (await tx
    .prepare(`SELECT id, lote_id FROM STOCK_OVINO_REGISTRO`)
    .all()) as { id: number; lote_id: number }[];
  for (const row of regRows) {
    if (!loteIds.has(row.lote_id)) continue;
    await tx.prepare(`DELETE FROM STOCK_OVINO_REGISTRO WHERE id = ?`).run(row.id);
    lecturas_eliminadas += 1;
  }
  ...
  for (const clave of clavesScope) {
    const res = await tx
      .prepare(`DELETE FROM STOCK_OVINO_DISPOSITIVO_HISTORIAL WHERE clave = ?`)
      .run(clave);
  ...
  for (const clave of clavesScope) {
    const res = await tx
      .prepare(`DELETE FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`)
      .run(clave);
...
function infoDesdePayload(
  payload: StockOvinaBackupPayload | null,
  creado_en: string | null
): StockOvinaBackupInfo {
...
export async function infoStockOvinaBackup(
  db: Db,
  cuentaId: number
): Promise<StockOvinaBackupInfo> {
  await ensureStockOvinaBackupTable(db);
  const row = (await db
    .prepare(`SELECT payload, creado_en FROM STOCK_OVINO_BACKUP WHERE cuenta_id = ?`)
    .get(cuentaId)) as { payload: string; creado_en: string } | undefined;
  if (!row?.payload) {
    return infoDesdePayload(null, null);
  }
  try {
    const payload = JSON.parse(row.payload) as StockOvinaBackupPayload;
...
export async function restaurarStockOvinaDesdeBackup(
  db: Db,
  cuentaId: number
): Promise<RestaurarStockOvinaResult> {
  return db.transaction(async (tx) => {
    await ensureStockOvinaBackupTable(tx);
    const row = (await tx
      .prepare(`SELECT payload FROM STOCK_OVINO_BACKUP WHERE cuenta_id = ?`)
    ...
    const lotesValidos = new Set(
      (
        (await tx.prepare(`SELECT id FROM STOCK_OVINO_LOTE`).all()) as { id: number }[]
      ).map((l) => l.id)
    );
    ...
    for (const grupo of enBloques(registrosValidos)) {
      ...
      await tx
        .prepare(
          `INSERT INTO STOCK_OVINO_REGISTRO (lote_id, eid, vid, fecha, hora, condicion)
           VALUES ${filas.join(", ")}`
        )
        .run(valores);
...
/** Elimina todo el stock ovino de una cuenta (o global si cuentaId es null). */
export async function vaciarStockOvinaCompleto(
  db: Db,
  ...
  if (cuentaId != null) {
    return db.transaction(async (tx) => {
      const payload = await gatherBackupPayloadForCuenta(tx, cuentaId);
      if (payload) {
        await crearBackupStockOvina(tx, cuentaId);
      }
      return vaciarStockOvinaDeCuenta(tx, cuentaId);
    });
  }
  ...
  return db.transaction(async (tx) => {
    const counts = (await tx.prepare(
      `SELECT
         (SELECT COUNT(*)::int FROM STOCK_OVINO_REGISTRO) AS lecturas,
         (SELECT COUNT(*)::int FROM STOCK_OVINO_DISPOSITIVO WHERE clave <> '__meta__') AS dispositivos,
    ...
    await tx.prepare(`TRUNCATE TABLE STOCK_OVINO_REGISTRO`).run();
    await tx.prepare(`TRUNCATE TABLE STOCK_OVINO_DISPOSITIVO_HISTORIAL`).run();
    await tx.prepare(`TRUNCATE TABLE STOCK_OVINO_DISPOSITIVO`).run();
...
async function registroMetaPorClave(
  ...
  const rows = (await db
    .prepare(`SELECT eid, vid, fecha FROM STOCK_OVINO_REGISTRO`)
...
export async function getEstadoDispositivoStock(
  ...
  const claveNorm = normalizarClaveDispositivo(clave);
  const row = (await db
    .prepare(`SELECT estado FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`)
...
export async function countStockOvinaDispositivosActivos(
  db: Db,
  filters?: StockOvinoFilters
): Promise<number> {
  const [dispositivos, ventasClaves] = await Promise.all([
    listStockOvinaDispositivos(db, filters),
...
async function resolverClavesRegistradasPorNumero(
  ...
  const rows = (await db
    .prepare(
      `SELECT DISTINCT eid, vid FROM STOCK_OVINO_REGISTRO
...
async function eidParaClaveRegistrada(
  ...
  const dispositivo = (await db
    .prepare(`SELECT eid FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`)
...
export async function updateStockOvinaDispositivoSexo(
  ...
  const eidGuardar = eid.trim() || claveNorm;
  await db.prepare(
    `INSERT INTO STOCK_OVINO_DISPOSITIVO (clave, eid, sexo, actualizado_en)
...
export async function updateStockOvinaDispositivoEdad(
  ...
  const row = (await db
    .prepare(
      `SELECT nacimiento_mes, nacimiento_anio FROM STOCK_OVINO_DISPOSITIVO WHERE clave = ?`
    )
    .get(claveNorm)) as
    | { nacimiento_mes: number | null; nacimiento_anio: number | null }
    | undefined;
  ...
  const eidGuardar = eid.trim() || claveNorm;
  await db.prepare(
    `INSERT INTO STOCK_OVINO_DISPOSITIVO (clave, eid, edad, actualizado_en)
...
export async function listStockOvinoLotes(
  db: Db,
  filters?: StockOvinoFilters
): Promise<StockOvinoLote[]> {
  let sql = `SELECT * FROM STOCK_OVINO_LOTE WHERE 1=1`;
  ...
  return (await db.prepare(sql).all(params)) as StockOvinoLote[];
}

export async function getStockOvinoLoteById(
  db: Db,
  id: number
): Promise<StockOvinoLote | undefined> {
  return (await db
    .prepare("SELECT * FROM STOCK_OVINO_LOTE WHERE id = ?")
    .get(id)) as StockOvinoLote | undefined;
}

export async function listStockOvinoRegistros(
  db: Db,
  filters?: StockOvinoFilters
): Promise<StockOvinoRegistro[]> {
  let sql = "SELECT * FROM STOCK_OVINO_REGISTRO WHERE 1=1";
  const params: Record<string, string | number> = {};
  sql = appendRegistroFilters(sql, params, filters);

  sql += " ORDER BY fecha DESC, hora DESC, id DESC";
  let rows = (await db.prepare(sql).all(params)) as StockOvinoRegistro[];
...
export async function getStockOvinoEstadisticas(
  db: Db,
  filters?: StockOvinoFilters
  ...
  const detalle_repetidos: StockOvinoEidRepetido[] = [];
...
export async function importStockOvinoRows(
  ...
  return db.transaction(async (tx) => {
    ...
    const lote = await tx
      .prepare(
        `INSERT INTO STOCK_OVINO_LOTE (nombre_archivo, filas, cuenta_id) VALUES (@nombre_archivo, @filas, @cuenta_id)`
      )
      .run({
    ...
    const upsertEmpresaDispositivo = await tx.prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO (clave, eid, empresa, sexo, estado)
       VALUES (@clave, @eid, @empresa, @sexo, 'VIVO')
       ON CONFLICT (clave) DO UPDATE SET
         eid = excluded.eid,
         empresa = CASE WHEN excluded.empresa <> '' THEN excluded.empresa
                        ELSE STOCK_OVINO_DISPOSITIVO.empresa END,
         sexo = CASE WHEN excluded.sexo <> '' THEN excluded.sexo
                     ELSE STOCK_OVINO_DISPOSITIVO.sexo END,
         actualizado_en = NOW()`
    );
    for (const r of rows) {
...
export interface AltaOvinoGenericaInput {
  cantidad: number;
  sexo: DispositivoSexo;
   ...
   * true = Capón, false = Carnero.
   */
  castrado?: boolean | null;
  potrero: string;
  empresa: string;
  /** Opcional. Si viene, se guarda y se agrega al catálogo de razas ovinas. */
  raza?: string | null;
  /** Opcional. Si viene, se guarda y se agrega al catálogo de pelajes ovinos. */
  pelaje?: string | null;
}

export interface AltaOvinoGenericaResult {
  creados: number;
  claves: string[];
  desde: string;
  hasta: string;
...
export async function crearOvinosGenericos(
  db: Db,
  input: AltaOvinoGenericaInput,
  cuentaId?: number | null,
  autor?: HistorialAutor
): Promise<AltaOvinoGenericaResult> {
  ...
  if (sexo !== "MACHO" || edad === null || edad < OVINO_FRONTERA_ADULTO) {
    castradoOpt = null;
  }

  const categoria: CategoriaOvino | "" =
  ...
  return db.transaction(async (tx) => {
    const claves = await reservarIdsOvinos(tx, cantidad);
    ...
    const condicionRegistro = categoria
      ? CATEGORIA_OVINO_LABELS[categoria]
    ...
    const insDisp = await tx.prepare(
      `INSERT INTO STOCK_OVINO_DISPOSITIVO (
         ...
         raza, pelaje, edad, nacimiento_mes, nacimiento_anio, observaciones, estado, tipo_baja, numero_guia,
         baja_mes, baja_anio, actualizado_en
       ) VALUES (
         @clave, @eid, @sexo, @empresa, @grupo, '', @potrero, @categoria, @castrado, 'generico',
         @raza, @pelaje, @edad, @nacimiento_mes, @nacimiento_anio, '', 'VIVO', '', '',
         NULL, NULL, NOW()
       )
       ON CONFLICT (clave) DO UPDATE SET
         eid = excluded.eid,
    ...
    for (const clave of claves) {
      const { eid, vid } = splitOvinoIdInterno(clave);
      await insReg.run({
        lote_id,
        eid,
        vid,
...
export interface AltaOvinoCabanaInput {
  rp: string;
  nombre_animal: string;
...
export interface AltaOvinoCabanaResult {
  clave: string;
  lote_id: number;
  categoria: CategoriaOvino;
  rp: string;
...
export async function crearOvinoCabana(
  db: Db,
  input: AltaOvinoCabanaInput,
  cuentaId?: number | null,
  autor?: HistorialAutor
): Promise<AltaOvinoCabanaResult> {
  ...
  let castradoOpt: boolean | null =
    input.castrado === true ? true : input.castrado === false ? false : null;
  if (sexo === "MACHO" && edad >= OVINO_FRONTERA_ADULTO && castradoOpt === null) {
    throw new Error(
      "Para machos de 36 meses o más indicá si es Capón (castrado) o Carnero."
    );
  }
  if (sexo !== "MACHO" || edad < OVINO_FRONTERA_ADULTO) {
    castradoOpt = null;
  }
  ...
  return db.transaction(async (tx) => {
    const claves = await reservarIdsOvinos(tx, 1);
    ...
    await tx
      ...
      .run({
        ...
        fecha,
        hora,
        condicion: `Cabaña · ${CATEGORIA_OVINO_LABELS[categoria]}`,
      });
...
export async function deleteStockOvinoLote(db: Db, id: number): Promise<boolean> {
  return db.transaction(async (tx) => {
    await tx.prepare("DELETE FROM STOCK_OVINO_REGISTRO WHERE lote_id = ?").run(id);
    return (await tx.prepare("DELETE FROM STOCK_OVINO_LOTE WHERE id = ?").run(id)).changes > 0;
  });
}

export async function countStockOvinoRegistros(
  db: Db,
  filters?: StockOvinoFilters
): Promise<number> {
  let sql = `SELECT COUNT(*) AS n FROM STOCK_OVINO_REGISTRO WHERE 1=1`;
  const params: Record<string, string | number> = {};
  sql = appendRegistroFilters(sql, params, filters);
...
export interface StockOvinaDispositivo {
  clave: string;
  eid: string;
  vid: string;
  sexo: DispositivoSexo;
...
export interface StockOvinaLecturaDetalle extends StockOvinoRegistro {
  nombre_archivo: string;
}

export interface StockOvinaDispositivoDetalle extends StockOvinaDispositivo {
  lecturas: StockOvinaLecturaDetalle[];
...
async function mapUltimaLecturaPorClave(db: Db): Promise<Map<string, LecturaResumen>> {
  const rows = (await db
    .prepare(
      `SELECT id, eid, vid, fecha, hora, condicion FROM STOCK_OVINO_REGISTRO`
    )
    .all()) as StockOvinoRegistro[];
...
function buildDispositivosFromRegistros(
  registros: StockOvinoRegistro[],
  repetidasClaves: Set<string>
): StockOvinaDispositivo[] {
  const map = new Map<string, StockOvinaDispositivo>();
...
export async function listStockOvinaDispositivos(
  db: Db,
  filters?: StockOvinoFilters
): Promise<StockOvinaDispositivo[]> {
  const registros = await listStockOvinoRegistros(db, filters);
...
export async function getStockOvinaDispositivoDetalle(
  db: Db,
  clave: string,
  filters?: StockOvinoFilters
): Promise<StockOvinaDispositivoDetalle | undefined> {
  ...
  const rows = (await db
    .prepare(
      `SELECT r.*, l.nombre_archivo
       FROM STOCK_OVINO_REGISTRO r
       JOIN STOCK_OVINO_LOTE l ON l.id = r.lote_id
       WHERE 1=1`
    )
    .all()) as StockOvinaLecturaDetalle[];
...
export async function countStockOvinaDispositivos(
  db: Db,
  filters?: StockOvinoFilters
): Promise<number> {
  return (await listStockOvinaDispositivos(db, filters)).length;
}
...
export interface StockOvinaDispositivoHistorial {
  id: number;
  clave: string;
  campo: string;
  etiqueta: string;
...
async function registrarHistorialCambiosDispositivo(
  ...
  await insertHistorialFila(
    ...
    "raza",
    "Raza",
    normalizarRazaOvino(anterior?.raza) || "—",
    normalizarRazaOvino(nuevo.raza) || "—",
    undefined,
    autor
  );
  await insertHistorialFila(
    ...
    "pelaje",
    "Pelaje",
    normalizarPelajeOvino(anterior?.pelaje) || "—",
    normalizarPelajeOvino(nuevo.pelaje) || "—",
    undefined,
    autor
  );
  await insertHistorialFila(
...
export async function listStockOvinaDispositivoHistorial(
  db: Db,
  clave: string
  ...
  return (await db
    ...
    .all(claveNorm)) as StockOvinaDispositivoHistorial[];
}
