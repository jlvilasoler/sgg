...
export const BROU_CAMPO_LABELS: Record<BrouCampoId, string> = {
  ...
  beneficiario_direccion: "Dirección",
...
function isGastoDestinoId(value: string): value is GastoDestinoId {
  return (GASTO_DESTINO_IDS as readonly string[]).includes(value);
}

function isBrouCampoId(value: string): value is BrouCampoId {
  return (BROU_CAMPO_IDS as readonly string[]).includes(value);
}

function mapeoValorAEtiqueta(valor: string): string {
  const t = valor.trim();
  if (!t) return "";
...
const COMISION_HEREDAR_IDS: GastoCampoId[] = [
  "empresa",
  "rubro",
  "sub_rubro",
  "responsable_gasto",
...
export function normalizeComisionConfig(raw: unknown): ComisionDocumentoConfig {
  ...
  const heredar = normalizeGastoCampoList(o.heredar).filter((c) => COMISION_HEREDAR_IDS.includes(c));
