...
import { fetchStockOvinaDispositivoHistorial } from "../../api";
import { useHeaderBackStep } from "../../header-back";
import type { StockOvinaDispositivoHistorial } from "../../types";
...
import { fmtRegOvino } from "./stock-ovina-utils";
...
export default function StockOvinaHistorialCambiosPanel({
  clave,
  vid,
  eid,
  apiOnline,
  ...
  const [filas, setFilas] = useState<StockOvinaDispositivoHistorial[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchStockOvinaDispositivoHistorial(clave);
      setFilas(data);
    } catch (e) {
  ...
  const backDestination =
    volverLabel.replace(/^Volver al?\s+/i, "").trim() || "Stock Ovino";
  ...
  return (
    <SubseccionInlinePanel
      ...
      description={
        <>
          Auditoría de ficha · REG {fmtRegOvino(eid, vid) || "—"} · Clave {clave || "—"}
        </>
      }
