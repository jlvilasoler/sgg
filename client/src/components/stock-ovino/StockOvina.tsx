...
import {
  deleteStockOvinaDispositivos,
  fetchEmpresasOperativasStock,
  fetchStockOvinaDispositivos,
  fetchStockOvinaSalidas,
  fetchStockOvinaVentasDispositivos,
} from "../../api";
import { useHeaderBackStep } from "../../header-back";
import type { AuthUser, DispositivoEstado, StockOvinaDispositivo } from "../../types";
...
import StockOvinaDashKpi from "./StockOvinaDashKpi";
import StockOvinaBulkPanel from "./StockOvinaBulkPanel";
import StockOvinaDetalle from "./StockOvinaDetalle";
import StockOvinaEdadMiniTimeline from "./StockOvinaEdadMiniTimeline";
import StockOvinaEditarPanel from "./StockOvinaEditarModal";
import StockOvinaHistorialCambiosPanel from "./StockOvinaHistorialCambiosPanel";
import StockOvinaFiltrosSidebar from "./StockOvinaFiltrosSidebar";
import type {
  CategoriaFiltroKey,
  EdadFiltroKey,
  StockOvinaSortDir,
...
import {
  ...
  compareStockOvinaDispositivos,
  defaultStockOvinaSortDir,
  dispositivoSinFechaNacimiento,
  dispositivoActivoEnStock,
  esDispositivoFueraDeStock,
  calcularResumenStockOvinaKpis,
...
import {
  ...
  filtrosCacheKey,
  readStockOvinaPageCache,
  rowsDesdeCache,
  stockOvinaCacheScope,
  ventasClavesDesdeCache,
  writeStockOvinaPageCache,
} from "./stock-ovina-page-cache";
...
function fmtSexo(sexo: StockOvinaDispositivo["sexo"]): string {
  return sexo || "—";
}

function claseCeldaSexo(sexo: StockOvinaDispositivo["sexo"]): string {
  if (sexo === "MACHO") return "stock-td--sexo-macho";
  if (sexo === "HEMBRA") return "stock-td--sexo-hembra";
  return "stock-td--sexo-na";
}

function esOvinoCabana(d: StockOvinaDispositivo): boolean {
  return (
...
function nombreOvinoCabana(d: StockOvinaDispositivo): string {
  return (
...
const SORT_HINTS: Record<StockOvinaSortKey, { asc: string; desc: string }> = {
...
function StockSortableTh({
  ...
  label: ReactNode;
  sortKey: StockOvinaSortKey;
  activeKey: StockOvinaSortKey | null;
  activeDir: StockOvinaSortDir;
  className?: string;
  onSort: (key: StockOvinaSortKey) => void;
}) {
  ...
  const nextDir = active
    ? activeDir === "asc"
      ? "desc"
      : "asc"
    : defaultStockOvinaSortDir(sortKey);
...
function StockOvina({
  ...
  const {
    ...
    itemsFiltrados,
    mostrarDashboard,
  } = useStockOvinoAsideSearch(hubNavItems);

  const esAdmin = currentUser?.rol === "admin";
  const cacheScope = currentUser ? stockOvinaCacheScope(currentUser) : "";
  const filtrosInicialesKey = filtrosCacheKey({});
  const cacheInicial = cacheScope ? readStockOvinaPageCache(cacheScope) : null;

  const [rows, setRows] = useState<StockOvinaDispositivo[]>(() =>
  ...
  const [editarDispositivo, setEditarDispositivo] =
    useState<StockOvinaDispositivo | null>(null);
  ...
  const [sortKey, setSortKey] = useState<StockOvinaSortKey | null>(null);
  const [sortDir, setSortDir] = useState<StockOvinaSortDir>("asc");
  ...
  const [statsRows, setStatsRows] = useState<StockOvinaDispositivo[]>(
  ...
  useHeaderBackStep(!!editarDispositivo, volverEditar, "Stock Ovino");
  useHeaderBackStep(!!bulkOpen, volverBulk, "Stock Ovino");
  useHeaderBackStep(!!historialCambios, volverHistorial, "Stock Ovino");
  useHeaderBackStep(!!detalleClave, volverDetalle, "Stock Ovino");
  ...
  useEffect(() => {
    ...
    const cache = readStockOvinaPageCache(cacheScope);
    if (!cache) {
      setRows([]);
      setStatsRows([]);
      setVentasClaves(new Set());
  ...
  const load = useCallback(async () => {
    if (!apiOnline) {
      if (!cacheScope || !readStockOvinaPageCache(cacheScope)) {
        setRows([]);
        setStatsRows([]);
        setVentasClaves(new Set());
      }
    ...
    try {
      const salidasRes = await fetchStockOvinaSalidas();
      const [dispositivos, todos, ventas] = await Promise.all([
        fetchStockOvinaDispositivos(filtros),
        fetchStockOvinaDispositivos({}),
        fetchStockOvinaVentasDispositivos(),
      ]);
  ...
  const resumenKpis = useMemo(
    () => calcularResumenStockOvinaKpis(statsRows, ventasClaves),
  ...
  const toggleSort = useCallback((key: StockOvinaSortKey) => {
    setSortKey((prev) => {
      if (prev === key) {
        setSortDir((d) => (d === "asc" ? "desc" : "asc"));
        return prev;
      }
      setSortDir(defaultStockOvinaSortDir(key));
      return key;
    });
    setPage(1);
  }, []);
  ...
  const sortedRows = useMemo(() => {
    ...
    next.sort((a, b) =>
      compareStockOvinaDispositivos(a, b, sortKey, sortDir, empresaLabel, fmtPotrero),
    );
    return next;
  ...
  const eliminarSeleccionados = async () => {
    ...
    const muestra = seleccionados
      .slice(0, 6)
      .map((d) => fmtRegOvino(d.eid, d.vid) || d.clave)
      .join(", ");
    const extra =
    ...
    if (!ok) return;
    try {
      const result = await deleteStockOvinaDispositivos(
  ...
  const actualizarFila = (actualizado: StockOvinaDispositivo) => {
    setRows((prev) => {
    ...
    setStatsRows((prev) => {
      const next = prev.map((r) => (r.clave === actualizado.clave ? actualizado : r));
      const cached = cacheScope ? readStockOvinaPageCache(cacheScope) : null;
  ...
  return (
    <div className="stock-ovino-devices-page">
      <div className="sg-hub sg-hub--devices">
        <aside className="sg-hub-aside sg-hub-aside--filters" aria-label="Filtros Stock Ovino">
          <div className="sg-hub-aside-brand">
            ...
            <div>
              ...
              <p className="sg-hub-aside-title">Stock Ovino</p>
            </div>
          </div>
        ...
        <main className="sg-hub-main sg-hub-main--devices">
                ...
                {mostrarCargaVacia
                  ...
                  : loading
                    ...
                    : filteredRows.length === 0
                      ? rows.length === 0
                        ? "No hay dispositivos ovinos registrados. Importá lecturas para armar el stock."
          ...
          {apiOnline && (
            <section className="sg-hub-panel sg-devices-dashboard" aria-label="Resumen del stock ovino">
              ...
              <div className="sg-hub-kpi-strip sg-devices-kpi-summary">
                <StockOvinaDashKpi
                  ...
                  hint="Caravanas electrónicas ovinas únicas en la base."
                  variant="total"
                  trend={
                ...
                <StockOvinaDashKpi
                  ...
                  hint={
                    ventasSimuladorCount > 0
                      ...
                      : "Ovinos registrados como vendidos"
                  }
                  variant="vendido"
          ...
          <section className="sg-hub-panel sg-hub-panel--devices-table" aria-label="Listado de dispositivos ovinos">
            ...
            {seleccionados.length > 0 && (
              <div className="stock-ovina-selection-bar">
                <span className="stock-ovina-selection-count">
                  <strong>{seleccionados.length}</strong> seleccionado
                  {seleccionados.length === 1 ? "" : "s"}
                </span>
                <div className="stock-ovina-selection-actions">
          ...
          <table className="data-table stock-ovina-table stock-table-pro">
            <thead>
              <tr>
            ...
            <tbody>
              {mostrarCargaVacia ? (
                ...
                rowsPagina.map((d) => {
                  ...
                  const cabana = esOvinoCabana(d);
                  const nombreCabana = nombreOvinoCabana(d);
                  const nombreMostrar =
                  ...
                  const reg = fmtRegOvino(d.eid, d.vid) || "—";
                  return (
                  <tr
                    key={d.clave}
                    className={`stock-ovina-row stock-table-pro-row stock-table-pro-row--clickable${
