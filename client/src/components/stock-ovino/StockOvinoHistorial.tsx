...
import {
  deleteStockOvinoLote,
  fetchStockOvinoLotes,
} from "../../api";
import type { StockOvinoLote } from "../../types";
...
export default function StockOvinoHistorial({
  apiOnline,
  onError,
  onSuccess,
  onVolver,
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const list = await fetchStockOvinoLotes();
      setLotes(list);
    } catch (e) {
          ...
          <table className="data-table stock-historial-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="empty">
                    Sin importaciones. Cargá un archivo desde «Alta de Ovinos».
                  </td>
                </tr>
              ) : (
                lotesPagina.map((l) => (
