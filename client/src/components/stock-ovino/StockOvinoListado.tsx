...
import {
  fetchStockOvinoEstadisticas,
  fetchStockOvinoLotes,
  fetchStockOvinoRegistros,
} from "../../api";
import type {
...
import { dispositivoClave, fmtRegOvino } from "./stock-ovina-utils";
...
export default function StockOvinoListado({
  apiOnline,
  onError,
  ...
  const [lotes, setLotes] = useState<StockOvinoLote[]>([]);
  const [rows, setRows] = useState<StockOvinoRegistro[]>([]);
  const [stats, setStats] = useState<StockOvinoEstadisticas | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [lotList, regs, estadisticas] = await Promise.all([
        ...
        fetchStockOvinoEstadisticas(filtros),
      ]);
      setLotes(lotList);
      setRows(regs);
          ...
          {stats.detalle_repetidos.map((item) => (
            <li key={item.clave}>
              <span className="stock-dash-detalle-eid">
                {fmtRegOvino(item.eid, item.vid || "") || item.clave}
              </span>
        ...
        <header className="stock-lecturas-hub-head-box stock-lecturas-hub-head-box--panel">
          <div className="stock-lecturas-hub-head-main">
            ...
            <p className="stock-lecturas-hub-sub muted">
              Altas ovinas de la cuenta: ID, categoría, origen, RP y ficha.
            </p>
          </div>
          {historialBtn}
        </header>
