...
import { fetchStockOvinaDispositivos, fetchStockOvinaVentasDispositivos } from "../../api";
import type { StockOvinaDispositivo } from "../../types";
import { dispositivoActivoEnStock } from "./stock-ovina-utils";
import {
...
interface Props {
  ...
  onSelect: (dispositivo: StockOvinaDispositivo) => void;
}

function coincideBusqueda(d: StockOvinaDispositivo, q: string): boolean {
...
function sublineOvino(d: StockOvinaDispositivo): string {
...
function textosBuscador(variant: Props["variant"]) {
  ...
  if (variant === "baja") {
    return {
      errorCargar: "Error al cargar ovinos activos",
      placeholder: "Buscar por REG, RP, nombre o registro…",
      toggleAbierto: "Cerrar listado",
      toggleCerrado: "Ver ovinos activos",
      metaLoading: "Cargando ovinos activos…",
      metaCount: (n: number, filtro?: string, busq?: number) =>
        `${n} ovino(s)${filtro ? ` · ${filtro}` : " activo(s)"}${
          busq != null ? ` · ${busq} coincidencia(s)` : ""
        }`,
      emptyBusqueda: "Sin coincidencias con el filtro y la búsqueda.",
      emptySinCoincidencias: "Sin coincidencias en el stock activo.",
      emptyCategoria: (label: string) => `No hay ovinos activos en ${label}.`,
      emptyGeneral: "No hay ovinos activos disponibles.",
    };
  }
  return {
    errorCargar: "Error al cargar ovinos activos",
    placeholder: "Buscar ovino activo por REG…",
    toggleAbierto: "Cerrar listado",
    toggleCerrado: "Ver ovinos activos",
    metaLoading: "Cargando ovinos activos…",
    metaCount: (n: number, filtro?: string, busq?: number) =>
...
export default function BuscadorCaravanaActiva({
  ...
  const [activos, setActivos] = useState<StockOvinaDispositivo[]>([]);
  ...
  const elegir = (d: StockOvinaDispositivo) => {
    onSelect(d);
    setBusqueda("");
    setAbierto(false);
  };
