...
import {
  fetchStockOvinaBackupInfo,
  fetchStockOvinoResumen,
  restaurarStockOvinaDesdeBackup,
  vaciarStockOvinaCompleto,
} from "../../api";
...
export default function StockOvinoAdmin({
  apiOnline,
  currentUser,
  onError,
  onSuccess,
  ...
  const restaurarBase = async () => {
    ...
    setRestaurando(true);
    try {
      const result = await restaurarStockOvinaDesdeBackup();
  ...
  const vaciarBase = async () => {
    ...
    const ok1 = await confirmAction({
      title: "Vaciar base de Stock Ovino",
    ...
    const ok2 = await confirmAction({
      ...
      message: `Última confirmación: se borrará por completo la base de Stock Ovino (${resumen.dispositivos_total} dispositivo(s), ${resumen.registros} lectura(s), ${resumen.lotes} importación(es)). Para continuar, escribí ELIMINAR.`,
    ...
    setVaciando(true);
    try {
      const result = await vaciarStockOvinaCompleto();
      clearStockOvinaPageCache();
      onSuccess(
    ...
    <div className="stock-admin-workspace">
      {!esAdmin ? (
        <p className="stock-admin-denied sg-hub-panel" role="status">
          Solo los administradores pueden administrar la base de Stock Ovino.
        </p>
      ) : (
