...
import {
  createStockOvinoRaza,
  deleteStockOvinoRaza,
  fetchStockOvinoRazas,
} from "../../api";
...
export default function SelectRazaOvinoDispositivo({
  value,
  onChange,
  disabled = false,
  id,
  ...
  const cargarCatalogo = useCallback(async () => {
    ...
    try {
      const razas = await fetchStockOvinoRazas();
  ...
  const guardarRazaNueva = async () => {
    ...
    try {
      const guardada = await createStockOvinoRaza(nombre);
  ...
  const eliminarRazaCatalogo = async () => {
    ...
    try {
      const eliminada = await deleteStockOvinoRaza(norm);
