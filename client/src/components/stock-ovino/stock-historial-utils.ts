...
export function grupoHistorialKey(item: StockOvinaDispositivoHistorial): string {
  return [
    item.creado_en,
    item.user_id ?? "",
    item.user_nombre ?? "",
...
export interface GrupoHistorialCambios {
  key: string;
  creado_en: string;
  items: StockOvinaDispositivoHistorial[];
  usuario: string;
  origen: string;
  origenLabel: string;
}
...
export function agruparHistorialCambios(
  filas: StockOvinaDispositivoHistorial[]
): GrupoHistorialCambios[] {
  const map = new Map<string, StockOvinaDispositivoHistorial[]>();
...
export function resumenHistorialCambios(filas: StockOvinaDispositivoHistorial[]) {
