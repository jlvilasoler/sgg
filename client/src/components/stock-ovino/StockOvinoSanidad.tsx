...
import type { AuthUser, StockOvinaDispositivo } from "../../types";
...
} from "../stock/StockControlSanitarioRegistroForm";
import {
  animalCategoriaLoteFromDispositivoOvino,
  animalIdFromDispositivoOvino,
} from "./stock-ovino-sanidad-dispositivo-utils";
import {
...
function agruparPor<T>(
  ...
  keyFn: (d: StockOvinaDispositivo) => T,
...
function buildUltimaLecturaGrupos(base: StockOvinaDispositivo[]): GrupoRapido[] {
  const map = new Map<string, { label: string; claves: string[] }>();
  for (const d of base) {
...
export default function StockOvinoSanidad({
  apiOnline,
  currentUser = null,
  onError,
  onSuccess,
  ...
  useHeaderBackStep(true, fichaProductoNombre ? volverDesdeFicha : onVolver, "Stock Ovino");
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const list = await fetchStockOvinaDispositivos({});
      let activos: StockOvinaDispositivo[];
      try {
        const ventas = await fetchStockOvinaVentasDispositivos();
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchStockControlSanitarioFechasAplicacion("ovino", clavesFiltradas)
      .then((data) => {
  ...
  const guardarMasivo = async () => {
    ...
    const items = seleccionados.map((d) => ({
      ...
      input: buildStockControlSanitarioInput(
        form,
        "fechas",
        animalCategoriaLoteFromDispositivoOvino(d),
        animalIdFromDispositivoOvino(d)
      ),
    }));
    ...
    try {
      const result = await createStockControlSanitarioBulk("ovino", items);
    ...
    <>
          ...
          >
            ...
            <div className="table-wrap table-wrap-stock-pro stock-sanidad-table-wrap">
              <table className="data-table stock-ovina-table stock-table-pro stock-sanidad-table-pro">
                <thead>
                  <tr>
