...
import { importStockOvinoBajaDispositivos, type TipoBaja } from "../../api";
...
/** Identificadores reales del ovino (genérico + cabaña). */
const CAMPOS_EQUINO = ["REG", "RP", "Nombre", "Registro"] as const;

const TIPOS_BAJA_HINTS: Partial<Record<TipoBaja, string>> = {
  VENTA_FRIGORIFICO: "Ovinos vendidos a frigorífico — estado Vendido",
...
export default function StockOvinoImportarBaja({
  apiOnline,
  onImported,
  onError,
  onSuccess,
  ...
  const [seleccion, setSeleccion] = useState<StockOvinaDispositivo | null>(null);
  ...
  const aplicarBajas = async (
    ...
    if (!items.length) {
      onError("Ingresá al menos un ovino");
      return;
    }
    ...
    setImporting(true);
    try {
      const r = await importStockOvinoBajaDispositivos(
        items.map((item) => ({
          numero: item.clave,
  ...
  const agregarPendiente = (e?: FormEvent) => {
    ...
    setPendientes((prev) => [
      ...prev,
      {
        ...
        etiqueta: etiquetaBajaOvino(seleccion),
        ...meta,
      },
    ]);
    setSeleccion(null);
  ...
  const manualPane = (
    <section className="stock-import-pane" aria-label="Baja por número de ovino">
      <form
        id={formId}
      ...
      >
        <div className={embedded ? "stock-baja-form-fields-box" : undefined}>
          <div className="field stock-import-field stock-import-field--numero">
            <label htmlFor={`${formId}-buscador`}>Ovino activo</label>
            ...
            {seleccion ? (
              ...
              <p className="stock-import-field-hint muted">
                ...
                genealógico. Solo ovinos con estado <strong>Vivo</strong> (genéricos y cabaña).
              </p>
            )}
          </div>
        ...
        >
          ...
          <button
            ...
            title="Suma el ovino a la lista para dar de baja varios juntos"
          >
          ...
          <button
            ...
            title="Da de baja solo el ovino seleccionado"
          >
            Confirmar baja
          </button>
        </div>
      ...
      {confirmadas.length > 0 ? (
        ...
        <p className="stock-import-queue-empty muted">
          Completá los datos de baja, elegí el ovino y pulsá <strong>Confirmar baja</strong>.
      ...
      <div className="stock-baja-hub-workspace">
        <section className="stock-baja-hub-box" aria-label="Guía de baja">
          <header className="stock-baja-hub-head-box">
            ...
            <h2 className="stock-baja-hub-title">Ovinos fuera del stock</h2>
            <p className="stock-baja-hub-sub muted">
              Marcá ovinos como vendidos, frigorífico, muerte u otras salidas. Salen del stock
    ...
    <div className="card stock-import-shell stock-import-shell--baja">
      <div className="form-header stock-import-head">
        <PageModuleHeadRow
          ...
          subtitle="Marcá ovinos como vendidos, frigorífico, muerte u otras salidas. Solo por número (REG, RP, nombre o registro)."
        />
      </div>
        ...
        >
          ...
          <div className="stock-facet-group">
            ...
            <p className="stock-import-sidebar-note">
              ...
              <strong>observaciones</strong>. Solo aparecen ovinos <strong>vivos</strong>.
            </p>
          </div>
        </aside>

        <div className="stock-ovina-main stock-import-main">{manualPane}</div>
      </div>
    </div>
  );
