...
/** REG ovino: prefijo EID y número VID juntos (ej. 199-000000000000000010). */
export function fmtRegOvino(eid: string, vid = ""): string {
  const { eid: e, vid: v } = splitEidVid(eid, vid);
  if (e && v) return `${e}-${v}`;
  if (v) return v;
  if (e) return e;
...
/** True si el ovino tiene ficha de cabaña (RP / nombre / origen). */
export function esOvinoCabana(d: {
...
export function etiquetaBajaOvino(d: {
  eid: string;
  vid: string;
  clave: string;
  nombre_animal?: string | null;
...
export const MACHO_FRONTERA_TERNERO = MACHO_FRONTERA_CORDERO;
/** @deprecated Usar MACHO_FRONTERA_BORREGO */
export const MACHO_FRONTERA_NOVILLO = MACHO_FRONTERA_BORREGO;

export interface EtapaEvolucionMacho {
  id: "CORDERO" | "BORREGO" | "ADULTO";
  titulo: string;
  rango: string;
  desdeMes: number;
  hastaMes: number;
...
export const ETAPAS_EVOLUCION_MACHO: EtapaEvolucionMacho[] = [
  {
    ...
    rango: "0 – 12 meses",
    desdeMes: 0,
    hastaMes: MACHO_FRONTERA_CORDERO,
  },
  {
    id: "BORREGO",
    titulo: "Borrego",
    rango: "12 – 36 meses",
    desdeMes: MACHO_FRONTERA_CORDERO,
    hastaMes: MACHO_FRONTERA_BORREGO,
  },
  {
    id: "ADULTO",
...
export function etapaMachoDesdeMeses(meses: number): EtapaEvolucionMacho {
  if (meses < MACHO_FRONTERA_CORDERO) return ETAPAS_EVOLUCION_MACHO[0];
  if (meses < MACHO_FRONTERA_BORREGO) return ETAPAS_EVOLUCION_MACHO[1];
...
export const HEMBRA_FRONTERA_TERNERA = HEMBRA_FRONTERA_CORDERA;
/** @deprecated */
export const HEMBRA_FRONTERA_VAQUILLONA = 24;
/** @deprecated Usar HEMBRA_FRONTERA_BORREGA */
export const HEMBRA_FRONTERA_VAQUILLONA_MAS_2 = HEMBRA_FRONTERA_BORREGA;

export interface EtapaEvolucionHembra {
  id: "CORDERA" | "BORREGA" | "OVEJA";
  titulo: string;
  rango: string;
  desdeMes: number;
  hastaMes: number;
...
export const ETAPAS_EVOLUCION_HEMBRA: EtapaEvolucionHembra[] = [
  {
    ...
    rango: "0 – 12 meses",
    desdeMes: 0,
    hastaMes: HEMBRA_FRONTERA_CORDERA,
  },
  {
    id: "BORREGA",
    titulo: "Borrega",
    rango: "12 – 36 meses",
    desdeMes: HEMBRA_FRONTERA_CORDERA,
    hastaMes: HEMBRA_FRONTERA_BORREGA,
  },
  {
    id: "OVEJA",
    titulo: "Oveja",
...
export function etapaHembraDesdeMeses(meses: number): EtapaEvolucionHembra {
  if (meses < HEMBRA_FRONTERA_CORDERA) return ETAPAS_EVOLUCION_HEMBRA[0];
  if (meses < HEMBRA_FRONTERA_BORREGA) return ETAPAS_EVOLUCION_HEMBRA[1];
...
export function pctHembraVisual(meses: number): number {
  ...
  if (m <= HEMBRA_FRONTERA_CORDERA) {
    return (m / HEMBRA_FRONTERA_CORDERA) * HEMBRA_SEGMENTO_PCT;
  }
  if (m <= HEMBRA_FRONTERA_BORREGA) {
    const t =
      (m - HEMBRA_FRONTERA_CORDERA) /
      (HEMBRA_FRONTERA_BORREGA - HEMBRA_FRONTERA_CORDERA);
    return HEMBRA_SEGMENTO_PCT + t * HEMBRA_SEGMENTO_PCT;
  }
  const t =
    (m - HEMBRA_FRONTERA_BORREGA) /
    (HEMBRA_ESCALA_MAX_MESES - HEMBRA_FRONTERA_BORREGA);
...
export function pctMachoVisual(meses: number): number {
  ...
  if (m <= MACHO_FRONTERA_CORDERO) {
    return (m / MACHO_FRONTERA_CORDERO) * MACHO_SEGMENTO_PCT;
  }
  if (m <= MACHO_FRONTERA_BORREGO) {
    const t =
      (m - MACHO_FRONTERA_CORDERO) /
      (MACHO_FRONTERA_BORREGO - MACHO_FRONTERA_CORDERO);
    return MACHO_SEGMENTO_PCT + t * MACHO_SEGMENTO_PCT;
  }
  const t =
    (m - MACHO_FRONTERA_BORREGO) /
    (MACHO_ESCALA_MAX_MESES - MACHO_FRONTERA_BORREGO);
...
export function progresoEtapaMacho(meses: number): number {
  const m = Math.min(Math.max(0, meses), MACHO_ESCALA_MAX_MESES);
  if (m <= MACHO_FRONTERA_CORDERO) {
    return (m / MACHO_FRONTERA_CORDERO) * 100;
  }
  if (m <= MACHO_FRONTERA_BORREGO) {
    return (
      ((m - MACHO_FRONTERA_CORDERO) /
...
export interface StockOvinaResumenKpis<T extends DispositivoResumenRow> {
  activos: T[];
  salidas: T[];
  ventasSimulador: T[];
  muertos: T[];
...
export function calcularResumenStockOvinaKpis<T extends DispositivoResumenRow>(
  dispositivos: ReadonlyArray<T>,
  ventasClaves: ReadonlySet<string>
): StockOvinaResumenKpis<T> {
  const activos: T[] = [];
  const salidas: T[] = [];
...
export function fmtSalidasSistemaHint(
  kpis: Pick<
    StockOvinaResumenKpis<DispositivoResumenRow>,
...
export function categoriasDispositivo(d: {
  ...
  if (d.sexo === "MACHO") {
    ...
    if (etapa.id === "CORDERO") return new Set(["CORDERO"]);
    if (etapa.id === "BORREGO") return new Set(["BORREGO"]);
    if (d.castrado === true) return new Set(["CAPON"]);
    if (d.castrado === false) return new Set(["CARNERO"]);
    return new Set(["CAPON", "CARNERO"]);
  }
...
export function fmtCategoriaOvino(categoria: string | null | undefined): string {
...
function nombreOvinoSortValue(d: {
...
/** Columnas ordenables de la tabla Stock Ovino. */
export type StockOvinaSortKey =
  | "reg"
  | "nombre"
  | "empresa"
  | "generacion"
...
export type StockOvinaSortDir = "asc" | "desc";

export function defaultStockOvinaSortDir(key: StockOvinaSortKey): StockOvinaSortDir {
...
export function compareStockOvinaDispositivos(
  a: {
    clave: string;
    eid: string;
    vid: string;
  ...
  switch (key) {
    case "reg":
      ...
      break;
    case "nombre":
      c = cmpTexto(nombreOvinoSortValue(a), nombreOvinoSortValue(b));
      break;
    case "empresa":
    ...
    case "potrero":
      ...
      break;
    case "categoria":
      c = cmpTexto(fmtCategoriaOvino(a.categoria), fmtCategoriaOvino(b.categoria));
      break;
    case "sexo":
