...
import { bulkPatchStockOvinaDispositivos, type EmpresaOperativaStock } from "../../api";
...
interface Props {
  ...
  onVolver: () => void;
  seleccionados: StockOvinaDispositivo[];
...
export default function StockOvinaBulkPanel({
  empresas,
  onVolver,
  seleccionados,
  totalFiltrados,
  ...
  useHeaderBackStep(true, onVolver, "Stock Ovino");
  ...
  const aplicar = async () => {
    ...
    setGuardando(true);
    try {
      const result = await bulkPatchStockOvinaDispositivos(
