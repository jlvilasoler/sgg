...
import {
  createStockGanaderoRaza,
  deleteStockGanaderoRaza,
  fetchStockGanaderoRazas,
} from "../../api";
...
interface Props {
  ...
  /** Solo superadministrador: puede eliminar razas del catálogo. */
  puedeEliminarRaza?: boolean;
  /** Razas de respaldo si la API no está disponible. */
  razasFallback?: readonly string[];
  selectClassName?: string;
  /** @deprecated Usa el mismo estilo que el select vía concepto-input-group. */
  otraInputClassName?: string;
}
...
export default function SelectRazaDispositivo({
  value,
  onChange,
  disabled = false,
  id,
  ...
  razasFallback = RAZAS_PREDEFINIDAS,
  selectClassName = "stock-edit-select",
}: Props) {
  const [catalogo, setCatalogo] = useState<string[]>(() => ordenarRazas(razasFallback));
  ...
  const cargarCatalogo = useCallback(async () => {
    if (!apiOnline) {
      setCatalogo(ordenarRazas(razasFallback));
      return;
    }
    setCargandoCatalogo(true);
    try {
      const razas = await fetchStockGanaderoRazas();
      setCatalogo(ordenarRazas(razas.length ? razas : razasFallback));
    } catch (e) {
      setCatalogo(ordenarRazas(razasFallback));
  ...
  }, [apiOnline, onError, razasFallback]);
  ...
  const guardarRazaNueva = async () => {
    ...
    try {
      const guardada = await createStockGanaderoRaza(nombre);
  ...
  const eliminarRazaCatalogo = async () => {
    ...
    const ok = await confirmAction({
      title: "Eliminar raza",
      message: `¿Eliminar «${norm}» del catálogo de razas?\n\nNo se borra de dispositivos ya guardados; solo deja de aparecer en el listado.`,
    ...
    try {
      const eliminada = await deleteStockGanaderoRaza(norm);
