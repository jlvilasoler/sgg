...
import type { CategoriaFiltroKey, EdadFiltroKey, StockGanaderaSortDir, StockGanaderaSortKey } from "./stock-ganadera-utils";
import {
  ...
  compareStockGanaderaDispositivos,
  defaultStockGanaderaSortDir,
...
function aplicaFacetas(
  ...
  return rows.filter((d) => {
    ...
    if (
      filtroPotrero.size > 0 &&
      !filtroPotrero.has(potreroFiltroKey(d.potrero ?? ""))
    ) {
      return false;
    }
...
function StockSortableTh({
  ...
  label: ReactNode;
  sortKey: StockGanaderaSortKey;
  activeKey: StockGanaderaSortKey | null;
  activeDir: StockGanaderaSortDir;
  className?: string;
  onSort: (key: StockGanaderaSortKey) => void;
}) {
  ...
  const nextDir = active
    ? activeDir === "asc"
      ? "desc"
      : "asc"
    : defaultStockGanaderaSortDir(sortKey);
...
export default function StockGanadera({
  ...
  const [sortKey, setSortKey] = useState<StockGanaderaSortKey | null>(null);
  const [sortDir, setSortDir] = useState<StockGanaderaSortDir>("asc");
  ...
  const [filtroPotrero, setFiltroPotrero] = useState<Set<string>>(() => new Set());
  ...
  const toggleSort = useCallback((key: StockGanaderaSortKey) => {
    setSortKey((prev) => {
      if (prev === key) {
        setSortDir((d) => (d === "asc" ? "desc" : "asc"));
        return prev;
      }
      setSortDir(defaultStockGanaderaSortDir(key));
      return key;
    });
    setPage(1);
  }, []);
  ...
  const facetCounts = useMemo(() => {
    ...
    const potrero: Record<string, number> = {};
    ...
    for (const d of rows) {
      ...
      const potreroKey = potreroFiltroKey(d.potrero ?? "");
      potrero[potreroKey] = (potrero[potreroKey] ?? 0) + 1;
  ...
  const potreroOpciones = useMemo(() => {
    const keys = Object.keys(facetCounts.potrero).filter(
      (k) => (facetCounts.potrero[k] ?? 0) > 0
    );
    keys.sort((a, b) => {
      if (a === "") return -1;
      if (b === "") return 1;
        ...
        <aside className="sg-hub-aside sg-hub-aside--filters" aria-label="Filtros Stock Ganadero">
          ...
          {apiOnline ? (
            <StockGanaderaFiltrosSidebar
              ...
              potreroOpciones={potreroOpciones}
              ...
              onTogglePotrero={(k) => setFiltroPotrero((p) => toggleSet(p, k))}
              ...
              onLimpiarPotrero={() => setFiltroPotrero(new Set())}
          ...
          <section className="sg-hub-panel sg-hub-panel--devices-table" aria-label="Listado de dispositivos">
            ...
            {(hayFacetasActivas ||
              ...
              <div className="sg-devices-chips" aria-label="Filtros activos">
                ...
                {[...filtroPotrero].map((k) => (
                  <span key={`potrero-${k || "sin"}`} className="stock-ganadera-chip">
                    Potrero: {labelPotreroFiltro(k)}
                    <button
                      type="button"
                      aria-label="Quitar filtro de potrero"
                      onClick={() => setFiltroPotrero((p) => toggleSet(p, k))}
                    >
                      ×
                    </button>
                  </span>
