...
import { fetchStockEquinaDispositivos, fetchStockEquinaVentasDispositivos } from "../../api";
import type { StockEquinaDispositivo } from "../../types";
import { dispositivoActivoEnStock } from "./stock-equina-utils";
import {
...
interface Props {
  ...
  onSelect: (dispositivo: StockEquinaDispositivo) => void;
}

function coincideBusqueda(d: StockEquinaDispositivo, q: string): boolean {
...
export default function BuscadorCaravanaActiva({
  ...
  const [activos, setActivos] = useState<StockEquinaDispositivo[]>([]);
  ...
  const elegir = (d: StockEquinaDispositivo) => {
    onSelect(d);
    setBusqueda("");
    setAbierto(false);
  };
