...
import { fetchStockEquinaDispositivoHistorial } from "../../api";
...
export default function StockEquinaHistorialCambiosPanel({
  clave,
  vid,
  eid,
  apiOnline,
  ...
  const [filas, setFilas] = useState<StockEquinaDispositivoHistorial[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchStockEquinaDispositivoHistorial(clave);
      setFilas(data);
    } catch (e) {
  ...
  const backDestination =
    volverLabel.replace(/^Volver al?\s+/i, "").trim() || "Stock Equino";
