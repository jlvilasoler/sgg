...
import { fetchEmpresasOperativasStock, fetchStockEquinaSalidas } from "../../api";
...
export default function StockEquinaSalidas({
  apiOnline,
  onError,
  onVolver,
  refreshKey = 0,
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { dispositivos, bajasReparadas: reparadas } = await fetchStockEquinaSalidas({
      ...
      {apiOnline && (
        <section className="stock-dash stock-dash--salidas" aria-label="Resumen de salidas">
          <div className="stock-dash-head">
            ...
            <p className="stock-dash-sub">Bajas registradas en el stock equino</p>
          </div>
          {resumenCards}
        </section>
      )}
