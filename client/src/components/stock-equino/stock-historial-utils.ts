import type { StockEquinaDispositivoHistorial } from "../../types";
...
export function grupoHistorialKey(item: StockEquinaDispositivoHistorial): string {
  return [
    item.creado_en,
    item.user_id ?? "",
    item.user_nombre ?? "",
...
export interface GrupoHistorialCambios {
  key: string;
  creado_en: string;
  items: StockEquinaDispositivoHistorial[];
  usuario: string;
  origen: string;
  origenLabel: string;
}
...
export function agruparHistorialCambios(
  filas: StockEquinaDispositivoHistorial[]
): GrupoHistorialCambios[] {
  const map = new Map<string, StockEquinaDispositivoHistorial[]>();
...
export function resumenHistorialCambios(filas: StockEquinaDispositivoHistorial[]) {
