...
import type { AuthUser, StockEquinaDispositivo } from "../../types";
...
import {
  animalCategoriaLoteFromDispositivoEquino,
  animalIdFromDispositivoEquino,
} from "./stock-equino-sanidad-dispositivo-utils";
import {
...
function agruparPor<T>(
  ...
  keyFn: (d: StockEquinaDispositivo) => T,
...
function buildUltimaLecturaGrupos(base: StockEquinaDispositivo[]): GrupoRapido[] {
  const map = new Map<string, { label: string; claves: string[] }>();
  for (const d of base) {
...
export default function StockEquinoSanidad({
  apiOnline,
  currentUser = null,
  onError,
  onSuccess,
  ...
  useHeaderBackStep(true, fichaProductoNombre ? volverDesdeFicha : onVolver, "Stock Equino");
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const list = await fetchStockEquinaDispositivos({});
      let activos: StockEquinaDispositivo[];
      try {
        const ventas = await fetchStockEquinaVentasDispositivos();
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchStockControlSanitarioFechasAplicacion("equino", clavesFiltradas)
      .then((data) => {
  ...
  const guardarMasivo = async () => {
    ...
    const items = seleccionados.map((d) => ({
      ...
      input: buildStockControlSanitarioInput(
        form,
        "fechas",
        animalCategoriaLoteFromDispositivoEquino(d),
        animalIdFromDispositivoEquino(d)
      ),
    }));
    ...
    try {
      const result = await createStockControlSanitarioBulk("equino", items);
    ...
    <>
          ...
          >
            ...
            <div className="table-wrap table-wrap-stock-pro stock-sanidad-table-wrap">
              <table className="data-table stock-equina-table stock-table-pro stock-sanidad-table-pro">
                <thead>
                  <tr>
