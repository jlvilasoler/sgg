...
import {
  CATEGORIAS_UNIDAD_OVINA,
  formatUnidadOvina,
  type GrupoUnidadOvina,
...
const GRUPO_LABEL: Record<GrupoUnidadOvina, string> = {
  Común: "Común",
  Hembra: "Hembras",
  Macho: "Machos",
};

const GRUPO_ORDER: GrupoUnidadOvina[] = ["Hembra", "Macho", "Común"];

function grupoClass(grupo: GrupoUnidadOvina): string {
  if (grupo === "Hembra") return "is-hembra";
  if (grupo === "Macho") return "is-macho";
  return "is-comun";
}

export default function ConfigDotacionOvina({
  onVolver,
  volverLabel = "Volver a Configuración SAG",
}: Props) {
  const filas = useMemo(() => [...CATEGORIAS_UNIDAD_OVINA], []);

  const resumen = useMemo(() => {
    const uos = filas.map((fila) => fila.uo);
    return {
      minUo: Math.min(...uos),
      maxUo: Math.max(...uos),
  ...
  return (
    <div className="subseccion-panel config-dotacion-ganadera config-dotacion-ovina">
      ...
      <section
        ...
        aria-labelledby="config-dotacion-ovina-title"
      >
        <header className="sg-hub-panel-head config-dotacion-ganadera-head">
          <div className="config-dotacion-ganadera-head-copy">
            <p className="sg-hub-panel-kicker">Ovinos · Referencia</p>
            <h2 id="config-dotacion-ovina-title" className="sg-hub-panel-title">
              Coeficientes UO por categoría
            </h2>
            <p className="config-dotacion-ganadera-lead muted">
              Equivalencias en unidades ovinas (UO) según el cronograma de evolución del stock
              ovino (hembra o macho por edad). Se usan en el Inicio (ovinos por potrero) para
              calcular la dotación en UO/ha.
            </p>
          </div>
          <span className="config-dotacion-ganadera-head-icon config-dotacion-ovina-head-icon" aria-hidden>
        ...
        <div className="config-dotacion-ganadera-kpis" aria-label="Resumen de coeficientes">
          ...
          <article className="config-dotacion-ganadera-kpi">
            <span className="config-dotacion-ganadera-kpi-label">Rango UO</span>
            <strong>
              {formatUnidadOvina(resumen.minUo)} – {formatUnidadOvina(resumen.maxUo)}
            </strong>
          </article>
          <article className="config-dotacion-ganadera-kpi config-dotacion-ganadera-kpi--base">
            ...
            <strong>1 UO = oveja / capón adulto</strong>
          </article>
        </div>
        ...
        <div className="config-dotacion-ganadera-intro">
          <div className="config-dotacion-ganadera-formula" role="note">
            ...
            <div>
              <strong>Dotación (UO/ha)</strong>
              <span>= Σ ovinos × UO de su categoría ÷ hectáreas del potrero</span>
            </div>
          </div>
        </div>
        ...
        <p className="config-dotacion-ganadera-foot muted">
          Las categorías siguen el mismo cronograma de la ficha de stock ovino: cordera/borrega/oveja
          y cordero/borrego/capón o carnero. Corderas y corderos aportan menos carga; carneros
          adultos aportan un poco más. Sin sexo o edad no aportan UO.
        </p>
      </section>
    </div>
  );
